...
def getViews(id):
  url = ("https://www.googleapis.com/youtube/v3/videos?part=snippet%2C+statistics&id="+id+"&maxResults=50&key="+c.DEVELOPER_KEY)
...
def youtube_search(options):
    youtube = build(c.YOUTUBE_API_SERVICE_NAME, c.YOUTUBE_API_VERSION,
      developerKey=c.DEVELOPER_KEY)
...
def getVideos(query,count):
  argparser.add_argument("--q", help="Search term", default=query)
  argparser.add_argument("--max-results", help="Max results", default=count)
